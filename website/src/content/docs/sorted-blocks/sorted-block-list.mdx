---
title: Sorted-Block-List<T>
description: 支持严格 O(1) 排名查询的有序分块列表。
sidebar:
  order: 2
---

`SortedBlockList` 是一种具有严格 *O*(1) 随机访问能力的有序容器。它完全是一个 [Block-List](../../more/block-list/) 的包装器，通过牺牲一部分插入性能，换取了在有序集合中快速的 Rank（按排名）查询能力。

{/* 如果你的应用场景中，根据排名获取元素 (kth) 的频率远高于 **插入/删除** 频率，`SortedBlockList` 是比 `SortedBlockArray` 更理想的选择，因为它提供了更稳定的 *O*(1) 访问性能。 */}
